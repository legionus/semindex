...
"$SEMINDEX" compiler --database "$symbols_db" --commands-database "$commands_db" -- \
	cc --no-default-config '-DJSON_VALUE="quoted value"' "$SOURCE_DIR/tests/test1.c"
"$SEMINDEX" compiler --database "$symbols_db" --commands-database "$commands_db" -- \
	cc --no-default-config "$SOURCE_DIR/tests/test.c"
"$SEMINDEX" compiler --variant=debug --database "$symbols_db" \
	--commands-database "$commands_db" -- cc --no-default-config -DDEBUG \
	"$SOURCE_DIR/tests/test.c"
...
if [ "$(sqlite3 :memory: "SELECT value FROM json_each(CAST(readfile('$output') AS TEXT), '\$[1].arguments') WHERE key = 2")" != '-DJSON_VALUE="quoted value"' ]; then
